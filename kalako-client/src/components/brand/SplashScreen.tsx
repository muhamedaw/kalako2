    ...
    >
      <defs>
        ...
        <linearGradient id="splashBar" x1="0" y1="0" x2="1" y2="0">
          ...
          <stop offset="50%" stopColor={COLORS.accentLime} />
        ...
        <style>{`
          ...
          @media (prefers-reduced-motion: reduce) { .sflA,.sflB,.sflC,.sflD,.splashLogo,.splashRing,.sDot,.sBarFill { animation: none !important; } }
        `}</style>
      </defs>
      ...
      <g opacity={0.6}>
        ...
        <ellipse className="sflB" cx={900} cy={900} rx={300} ry={280} fill={COLORS.accentLime} filter="url(#softBlob)" />
        <ellipse className="sflC" cx={220} cy={1500} rx={320} ry={260} fill={COLORS.accentPink} filter="url(#softBlob)" />
        <ellipse className="sflD" cx={880} cy={1700} rx={280} ry={220} fill={COLORS.primary} filter="url(#softBlob)" />
      </g>
      ...
      <g transform="translate(540, 820)" className="splashLogo">
        ...
        <g transform="translate(-110, -200)" filter="url(#glowGold)">
          <path d="M0,52 L26,0 L52,38 L78,0 L104,52 Z M0,52 L104,52 L104,66 L0,66 Z" fill={COLORS.primary} stroke="#FFFFFF" strokeWidth={3} strokeLinejoin="round" />
          <circle cx={26} cy={0} r={8} fill={COLORS.primary} />
          <circle cx={52} cy={25} r={6} fill={COLORS.accentLime} />
          <circle cx={78} cy={0} r={8} fill={COLORS.accentPink} />
        </g>
        <g filter="url(#glowGold)">
          <text x={0} y={120} textAnchor="middle" fontWeight={900} fontSize={320} fill="url(#gradLimeCoral)" fontFamily="Cairo, Tajawal, 'Noto Sans Arabic', system-ui, sans-serif">؟</text>
        </g>
        <text x={0} y={260} textAnchor="middle" fontWeight={900} fontSize={64} fill="#FFFFFF" filter="url(#glowGold)" letterSpacing="-2" fontFamily="Cairo, Tajawal, 'Noto Sans Arabic', system-ui, sans-serif">{t.splashTagline}</text>
        <circle r={280} fill="none" stroke={COLORS.accentLime} strokeWidth={2} strokeOpacity={0.4} className="splashRing" strokeDasharray="6 16" />
        <circle cx={280} cy={0} r={6} fill={COLORS.accentLime} filter="url(#glowLime)" />
      </g>

      <g transform="translate(540, 1480)">
        <circle cx={-44} cy={0} r={12} fill={COLORS.primary} filter="url(#glowGold)" className="sDot sDot1" />
        <circle cx={0} cy={0} r={12} fill={COLORS.accentLime} filter="url(#glowLime)" className="sDot sDot2" />
        <circle cx={44} cy={0} r={12} fill={COLORS.primary} filter="url(#glowGold)" className="sDot sDot3" />
      </g>

      <text x={540} y={1560} textAnchor="middle" fontWeight={700} fontSize={36} fill="#F5EDE4" opacity={0.85} fontFamily="Cairo, Tajawal, 'Noto Sans Arabic', system-ui, sans-serif">{t.loadingText}</text>

      {showProgress && (
        <g transform="translate(140, 1700)">
          <rect x={0} y={0} width={800} height={14} rx={7} fill={COLORS.bgEnd} stroke={COLORS.accentLime} strokeWidth={2} strokeOpacity={0.6} />
          <rect x={0} y={0} width={800} height={14} rx={7} fill="url(#splashBar)" filter="url(#glowLime)" className="sBarFill" />
        </g>
      )}

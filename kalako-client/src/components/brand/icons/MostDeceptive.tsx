...
export default function MostDeceptive({ size = 72, className }: Props) {
  ...
  return (
    <svg viewBox="0 0 72 72" width={size} height={size} className={className} role="img"         aria-label={t.mostDeceptiveAria}>
      ...
      {/* fox face */}
      <polygon points="36,16 20,8 14,28 36,34" fill={COLORS.primary} filter="url(#mdGlow)" />
      <polygon points="36,16 52,8 58,28 36,34" fill={COLORS.primary} filter="url(#mdGlow)" />
      <ellipse cx={36} cy={34} rx={18} ry={14} fill={COLORS.primary} />
      ...
      {/* crown */}
      <path d="M24,4 L28,14 L36,6 L44,14 L48,4 L48,16 L24,16 Z" fill={COLORS.primary} stroke="#FFFFFF" strokeWidth={1} filter="url(#mdGlow)" />
